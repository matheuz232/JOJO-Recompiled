...
Reference: https://psx-spx.consoledev.net/kernelbios/
...
Immediately before each guest CPU step, the runtime retains a candidate pre-step Status and ordinary resume PC/next-PC state. If that step returns an interrupt exception, the continuation captures exactly:

- GPR1..GPR31 from the post-exception CPU state, after exception-entry pending-load retirement;
- HI and LO from the post-exception CPU state;
- COP0 Status from immediately before exception entry;
- resume PC equal to the interrupt EPC;
- resume next PC equal to the normal successor of that non-delay-slot instruction;
- pending-load state as empty;
- delay-slot state as inactive.

The continuation does **not** restore COP0 Cause, COP0 EPC, COP2/GTE state, or `external_interrupt_pending` as part of B0:17. Cause remains diagnostic state from exception handling; the external interrupt lines are synchronized again from I_STAT/I_MASK by the normal runtime loop. COP2/GTE is not part of the documented BIOS ReturnFromException register restore.

Interrupts are already accepted only outside an active branch delay slot, so v0 does not synthesize a delay-slot resume case.
...
- restores saved GPR1..GPR31, HI, LO, and the pre-exception COP0 Status;
- restores PC to the interrupted instruction address and next PC to its normal successor;
- clears delay-slot and pending-load continuation artifacts;
- deactivates the interrupt continuation;
- leaves Cause/EPC/COP2 diagnostic or architectural state otherwise untouched;
...
- hook phase;
- saved interrupted GPR/HI/LO/Status/resume-PC values;
