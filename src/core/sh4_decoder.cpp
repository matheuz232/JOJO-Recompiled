...
Sh4Instruction decode_sh4(std::uint16_t raw, std::uint32_t address) noexcept {
    ...
    if (control_code == 0x405Au) { i.op = Sh4Op::lds_fpul_reg; i.rm = n_field(raw); return i; }
    if (control_code == 0x005Au) { i.op = Sh4Op::sts_fpul_reg; i.rn = n_field(raw); return i; }
    if (control_code == 0x4056u) { i.op = Sh4Op::lds_fpul_postinc; i.rm = n_field(raw); return i; }
    ...
    if (fpu_binary_code >= 0xF000u && fpu_binary_code <= 0xF003u) {
        ...
        if (fpu_binary_code == 0xF001u) i.op = Sh4Op::fsub;
        if (fpu_binary_code == 0xF002u) i.op = Sh4Op::fmul;
        if (fpu_binary_code == 0xF003u) i.op = Sh4Op::fdiv;
        i.rn = n_field(raw);
        i.rm = m_field(raw);
        return i;
    }
