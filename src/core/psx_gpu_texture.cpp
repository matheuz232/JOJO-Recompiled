#include "core/psx_gpu_texture.h"
...
[[nodiscard]] std::uint8_t apply_texture_window_axis(std::uint8_t coordinate,
                                                     std::uint32_t mask,
    ...
    return static_cast<std::uint8_t>((coordinate & static_cast<std::uint8_t>(~clear_mask)) |
                                     replacement);
}
...
[[nodiscard]] TextureCoordinate apply_texture_window(const PsxBus& bus,
                                                     std::uint8_t u,
...
[[nodiscard]] std::uint16_t sample_texture(const PsxBus& bus,
                                           std::uint8_t u,
                                           std::uint8_t v,
    ...
    if (depth == 0u) {
        const auto word_x = (page_x + static_cast<std::uint32_t>(adjusted.u) / 4u) & 0x3ffu;
        ...
        const auto palette_index = static_cast<std::uint32_t>((packed >> shift) & 0x0fu);
    ...
    if (depth == 1u) {
        const auto word_x = (page_x + static_cast<std::uint32_t>(adjusted.u) / 2u) & 0x3ffu;
        const auto packed = bus.gpu_vram[
            static_cast<std::size_t>(row) * PsxBus::gpu_vram_width + word_x];
        const auto shift = static_cast<std::uint32_t>(adjusted.u & 1u) * 8u;
        const auto palette_index = static_cast<std::uint32_t>((packed >> shift) & 0xffu);
        const auto clut_x = static_cast<std::uint32_t>(clut & 0x3fu) * 16u;
        const auto clut_y = static_cast<std::uint32_t>((clut >> 6u) & 0x1ffu);
        return bus.gpu_vram[
            static_cast<std::size_t>(clut_y) * PsxBus::gpu_vram_width +
    ...
    const auto word_x = (page_x + static_cast<std::uint32_t>(adjusted.u)) & 0x3ffu;
    return bus.gpu_vram[
        static_cast<std::size_t>(row) * PsxBus::gpu_vram_width + word_x];
}
...
void psx_gpu_execute_textured_rectangle(PsxBus& bus) noexcept {
    ...
    for (std::uint32_t row = 0u; row < height; ++row) {
        for (std::uint32_t column = 0u; column < width; ++column) {
            ...
            const auto pixel = sample_texture(bus, u, v, clut);
