...
std::uint32_t Ps1GpuState::gpu_stat() const noexcept {
    // Control-only GPUSTAT. Drawing-state bits stay at reset because GP0 is not modeled yet.
    std::uint32_t stat = (1u << 13) | (1u << 26) | (1u << 28);
    if (display_disabled_) stat |= 1u << 23;
    if (irq1_) stat |= 1u << 24;

    // GP1(08h) display-mode bits reflected by GPUSTAT.
    ...
    switch (dma_direction_ & 3u) {
        case 1u: dma_request = true; break;  // FIFO not full in the control-only model.
        case 2u: dma_request = true; break;  // Mirrors ready-to-receive DMA (bit 28).
        case 3u: dma_request = false; break; // Mirrors VRAM-to-CPU ready (bit 27), not modeled.
        default: break;
    }
