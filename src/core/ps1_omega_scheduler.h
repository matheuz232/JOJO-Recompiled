...
[[nodiscard]] inline std::vector<std::uint8_t> encode_ps1_omega_replay_descriptor(
    ...
    for (const auto& decision : descriptor.path) omega_scheduler_detail::write_decision(writer, decision);
    writer.u64(descriptor.assumption_chain.size());
    for (const auto& decision : descriptor.assumption_chain) omega_scheduler_detail::write_decision(writer, decision);
    return writer.take();
}
...
[[nodiscard]] inline std::optional<Ps1OmegaReplayDescriptor> decode_ps1_omega_replay_descriptor(
    ...
    if (path_count > 1000000u) return std::nullopt;
    descriptor.path.reserve(static_cast<std::size_t>(path_count));
    for (std::uint64_t i = 0; i < path_count; ++i) descriptor.path.push_back(omega_scheduler_detail::read_decision(reader));
    const auto assumption_count = reader.u64();
    if (assumption_count > 1000000u) return std::nullopt;
    descriptor.assumption_chain.reserve(static_cast<std::size_t>(assumption_count));
    for (std::uint64_t i = 0; i < assumption_count; ++i) descriptor.assumption_chain.push_back(omega_scheduler_detail::read_decision(reader));
