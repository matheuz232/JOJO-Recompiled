...
static void test_register_forms_round_trip_fpscr() {
    jojo::Sh4ReferenceState state{};
    state.r[1] = 0x000C0001u;
    ...
    CHECK(state.r[2] == 0x000C0001u);
}

static void test_memory_forms_round_trip_fpscr_little_endian() {
    ...
    CHECK(memory[14] == 0x0Cu);
    CHECK(memory[15] == 0x00u);
}

int main() {
    test_decoder_recognizes_fpscr_transfer_forms();
    test_register_forms_round_trip_fpscr();
    test_memory_forms_round_trip_fpscr_little_endian();
    if (failures) {
        std::cerr << failures << " SH-4 FPSCR transfer assertion(s) failed\n";
        return 1;
    }
    std::cout << "all SH-4 FPSCR transfer assertions passed\n";
    return 0;
}
