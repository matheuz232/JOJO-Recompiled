...
static void test_b0_0c_enableevent_matches_observed_handle_and_is_idempotent() {
    jojo::Ps1HleBios bios;
    jojo::R3000aState cpu{};
    ...
    const auto before_enable = bios.diagnostic_state_hash();
    cpu.pc = 0x000000B0u;
    cpu.next_pc = 0x000000B4u;
    ...
    CHECK(bios.dispatch(call(jojo::Ps1HleBiosDomain::b0, 0x0Cu, cpu), cpu).disposition ==
          jojo::Ps1HleBiosDisposition::handled);
    CHECK(cpu.gpr[2] == 1u);
    CHECK(cpu.pc == 0x80047D40u);
    const auto after_enable = bios.diagnostic_state_hash();
    CHECK(after_enable != before_enable);

    cpu.pc = 0x000000B0u;
    cpu.next_pc = 0x000000B4u;
    cpu.gpr[2] = 0u;
    cpu.gpr[31] = 0x80047D60u;
    CHECK(bios.dispatch(call(jojo::Ps1HleBiosDomain::b0, 0x0Cu, cpu), cpu).disposition ==
          jojo::Ps1HleBiosDisposition::handled);
    CHECK(cpu.gpr[2] == 1u);
    CHECK(bios.diagnostic_state_hash() == after_enable);

    cpu.pc = 0x000000B0u;
    cpu.next_pc = 0x000000B4u;
    cpu.gpr[4] = 0xF10000FFu;
    cpu.gpr[31] = 0x80047D80u;
    const auto before_invalid = bios.diagnostic_state_hash();
    CHECK(bios.dispatch(call(jojo::Ps1HleBiosDomain::b0, 0x0Cu, cpu), cpu).disposition ==
          jojo::Ps1HleBiosDisposition::handled);
    CHECK(cpu.gpr[2] == 1u);
    CHECK(cpu.pc == 0x80047D80u);
