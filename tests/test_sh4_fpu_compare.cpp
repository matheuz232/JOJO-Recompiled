...
static void test_double_precision_mode_is_rejected_without_mutating_t() {
    jojo::Sh4ReferenceState state{};
    state.fpscr |= 0x00080000u; // PR=1
    state.t = true;
    state.fr[0] = std::bit_cast<std::uint32_t>(1.0f);
    state.fr[1] = std::bit_cast<std::uint32_t>(1.0f);
    const auto run = run_word(0xF014u, state, 0x8C063000u);
    CHECK(!run);
    CHECK(state.t);
}
...
int main() {
    ...
    test_double_precision_mode_is_rejected_without_mutating_t();
    if (failures) {
