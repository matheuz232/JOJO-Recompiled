...
static void test_terminal_device_command_can_continue_without_device_effect() {
    auto runtime = make_runtime({
        test_mips::i(0x0Fu, 0u, 8u, 0x1F80u),
    ...
    options.mmio_event_capacity = 16u;
    const auto strict = runtime.run(options);
    CHECK(strict.stop_reason == jojo::Ps1BootStopReason::device_command_unimplemented);
    CHECK(strict.instructions_retired == 3u);
    CHECK(strict.last_pc == 0x8001000Cu);
    CHECK(strict.unsupported_access.has_value());
    if (strict.unsupported_access) {
        CHECK(strict.unsupported_access->physical_address == 0x1F801801u);
        CHECK(strict.unsupported_access->width == 1u);
        CHECK(strict.unsupported_access->write);
        CHECK(strict.unsupported_access->value == 0x0Au);
    }
...
static void test_infinity_scheduler_continues_past_device_command_frontier() {
    namespace fs = std::filesystem;
    const auto root = fs::temp_directory_path() / "jojo-omega-infinity-device-command-test";
    std::error_code ec;
    ...
    auto options = jojo::ps1_omega_infinity_options();
    options.epoch_retired_limit = 100u;
    options.instruction_quantum = 4u;
    options.hot_trace_capacity = 16u;
    options.max_session_disk_bytes = 32ull * 1024ull * 1024ull;

    jojo::Ps1OmegaInfinityControl control;
    const auto explored = jojo::explore_ps1_omega_infinity(
        executable, root, options, control,
        [&](const jojo::Ps1OmegaInfinityProgress& progress) {
            if (progress.total_retired >= 8u) control.request_stop();
        });
    CHECK(explored);
    if (explored) {
...
int main() {
    ...
    test_terminal_device_command_can_continue_without_device_effect();
    test_infinity_scheduler_continues_past_device_command_frontier();
